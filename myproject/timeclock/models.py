...
class PasswordResetAttempt(models.Model):
    """
    Tracks password reset and login attempts by IP address to implement rate limiting and bans.
    Ban levels:
    - Level 1: 10+ attempts in 1 minute = 10 minute ban
    - Level 2: 20+ attempts in 1 hour = 1 hour ban
    - Level 3: 30+ attempts in 1 day = 1 day ban with 7 day grace period
    - Level 4: 50+ attempts in 1 week = permanent ban
    """
    ...
    # Only used if attempt_type is 'ban'
    ban_level = models.IntegerField(null=True, blank=True)
    ...
    @property
    def is_banned(self):
        if self.attempt_type != 'ban' or not self.ban_end:
            return False
        return timezone.now() <= self.ban_end

    def calculate_ban_duration(self):
        if self.ban_level == 1:
            return timedelta(minutes=10)
        elif self.ban_level == 2:
            return timedelta(hours=1)
        elif self.ban_level == 3:
            return timedelta(days=1)
        elif self.ban_level == 4:
            return timedelta(days=36500)  # ~100 years
        return None

    @classmethod
    def check_and_create_ban(cls, ip_address):
        now = timezone.now()
        
        # Get the most recent ban for this IP
        last_ban = cls.objects.filter(
            ip_address=ip_address,
            attempt_type='ban'
        ).order_by('-attempt_time').first()

        # If there's a Level 4 ban, it's permanent - no new attempts allowed
        if last_ban and last_ban.ban_level == 4:
            return last_ban

        # Count attempts in different time windows
        minute_attempts = cls.objects.filter(
            ip_address=ip_address,
            attempt_type__in=['password_reset', 'login'],
            attempt_time__gte=now - timedelta(minutes=1)
        ).count()

        hour_attempts = cls.objects.filter(
            ip_address=ip_address,
            attempt_type__in=['password_reset', 'login'],
            attempt_time__gte=now - timedelta(hours=1)
        ).count()

        day_attempts = cls.objects.filter(
            ip_address=ip_address,
            attempt_type__in=['password_reset', 'login'],
            attempt_time__gte=now - timedelta(days=1)
        ).count()

        week_attempts = cls.objects.filter(
            ip_address=ip_address,
            attempt_type__in=['password_reset', 'login'],
            attempt_time__gte=now - timedelta(weeks=1)
        ).count()

        # Get the current ban level (0 if no ban or expired)
        current_ban_level = last_ban.ban_level if (last_ban and last_ban.is_banned) else 0

        # Determine if we need to update the ban level
        new_ban_level = None

        # Check for Level 4 - Need Level 3 and total 50 attempts in a week
        if current_ban_level == 3 and week_attempts >= 50:
            new_ban_level = 4
        # Check for Level 3 - Need Level 2 and total 30 attempts in a day
        elif current_ban_level == 2 and day_attempts >= 30:
            new_ban_level = 3
        # Check for Level 2 - Need Level 1 and total 20 attempts in an hour
        elif current_ban_level == 1 and hour_attempts >= 20:
            new_ban_level = 2
        # Check for Level 1 - Need 10 attempts in a minute
        elif minute_attempts >= 10:
            new_ban_level = 1

        # Update or create ban if needed
        if new_ban_level:
            if last_ban:
                # Only update if the new level is higher
                if new_ban_level > last_ban.ban_level:
                    last_ban.ban_level = new_ban_level
                    last_ban.attempt_time = now
                    last_ban.ban_start = now
                    duration = last_ban.calculate_ban_duration()
                    if duration:
                        last_ban.ban_end = now + duration
                    last_ban.save()
                return last_ban
            else:
                ...
                ban = cls.objects.create(
                    ...
                    attempt_time=now,
                    attempt_type='ban',
                    ban_level=new_ban_level,
                    ban_start=now
                )
                duration = ban.calculate_ban_duration()
                if duration:
                    ban.ban_end = now + duration
                ban.save()
                return ban
        ...
        cls.objects.filter(
            ip_address=ip_address,
            attempt_type__in=['password_reset', 'login'],
            attempt_time__lt=now - timedelta(weeks=1)
        ).delete()

        # Return the current ban if still active, otherwise None
        return last_ban if last_ban and last_ban.is_banned else None

    def set_ban(self, level):
        self.ban_level = level
        self.ban_start = timezone.now()
        duration = self.calculate_ban_duration()
        if duration:
            self.ban_end = self.ban_start + duration
        self.save()

    def __str__(self):
        return f"IP: {self.ip_address} - {'Banned' if self.is_banned else 'Attempt'} at {self.attempt_time}"


class LoginAttempt(models.Model):
    """
    Tracks login attempts by IP address to implement rate limiting and bans.
    Uses the same ban level progression as PasswordResetAttempt:
    - Level 1: 10+ attempts in 1 minute = 10 minute ban
    - Level 2: 20+ attempts in 1 hour = 1 hour ban
    - Level 3: 30+ attempts in 1 day = 1 day ban
    - Level 4: 50+ attempts in 1 week = permanent ban
    """
    ip_address = models.GenericIPAddressField()
    attempt_time = models.DateTimeField(auto_now_add=True)
    attempt_type = models.CharField(max_length=10, choices=[
    ...
    class Meta:
        indexes = [
            models.Index(fields=['ip_address', 'attempt_time']),
            models.Index(fields=['ip_address', 'attempt_type']),
        ]
    ...
    def check_and_create_ban(cls, ip_address):
        ...
        if new_ban_level:
            if last_ban:
                ...
                # Create new ban record only if no previous ban exists
                ban = cls.objects.create(
                    ip_address=ip_address,
                    attempt_time=now,
                    attempt_type='ban',
